(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('holmesaggregatorApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-holmesaggregatorApp-error');
                var entityKey = httpResponse.headers('X-holmesaggregatorApp-params');
                if (errorHeader) {
